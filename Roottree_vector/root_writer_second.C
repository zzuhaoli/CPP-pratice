...
void root_writer_second()
{
     ...
     std::vector<classinfo> v_gradeclassinfo2={class4,class5,class6};
    ...
    tree1->Branch("tea_gradeID",&v_tea_gradeID);
    tree1->Branch("tea_classID",&v_tea_classID);
    tree1->Branch("tea_age", &v_tea_age);
    tree1->Branch("tea_gender", &v_tea_age);
    tree1->Branch("tea_number", &v_tea_worknumber);
    tree1->Branch("tea_heigth", &v_tea_heigth);
    tree1->Branch("tea_weigth", &v_tea_weigth);
    tree1->Branch("tea_subject", &v_tea_subject);
    tree1->Branch("tea_level", &v_tea_level);
    ...
    for(int i=0;i<nentry_grade;i++)
   {
      ...
      {
        int v_stuinfosize = v_grainfo[i].getstudentsinfo().size();
        int v_teainfosize = v_grainfo[i].getteachersinfo().size();
        auto v_sinfo = v_grainfo[i].getstudentsinfo();
        auto v_tinfo = v_grainfo[i].getteachersinfo();
        for(int n=0;n<v_stuinfosize;n++)
        {
            v_stu_gradeID.push_back(v_grainfo[i].getgradeID());
            v_stu_classID.push_back(v_grainfo[i].getclassID());
        ...
        {
            v_tea_gradeID.push_back(v_grainfo[i].getgradeID());
            v_tea_classID.push_back(v_grainfo[i].getclassID());
